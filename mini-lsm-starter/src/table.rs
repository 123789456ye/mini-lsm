...
impl BlockMeta {
    ...
    pub fn decode_block_meta(mut buf: impl Buf) -> Vec<BlockMeta> {
        ...
        for i in 0..len {
            ...
            let first_key = KeyBytes::from_bytes(buf.copy_to_bytes(first_len as usize));
            let last_len = buf.get_u16_le();
            let last_key = KeyBytes::from_bytes(buf.copy_to_bytes(last_len as usize));
...
impl SsTable {
    ...
    pub fn open(id: usize, block_cache: Option<Arc<BlockCache>>, file: FileObject) -> Result<Self> {
        ...
        let len = bloom_offset;

        let block_meta_offset = file.read(len - 4, 4)?.as_slice().get_u32() as usize;
    ...
    pub fn read_block(&self, block_idx: usize) -> Result<Arc<Block>> {
        ...
        let len = end - offset;
        let buf = self.file.read(offset, len)?;
    ...
    pub fn find_block_idx(&self, key: KeySlice) -> usize {
        let mut l = 0;
        let mut r = self.block_meta.len();
        while l < r {
            let m = (l + r) / 2;
            if self.block_meta[m].first_key.as_key_slice() <= key {
                l = m + 1;
            } else {
                r = m;
            }
