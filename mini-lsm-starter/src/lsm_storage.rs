...
use crate::key::{self, KeySlice, TS_MAX, TS_RANGE_BEGIN, TS_RANGE_END};
...
impl LsmStorageInner {
    ...
    pub(crate) fn open(path: impl AsRef<Path>, options: LsmStorageOptions) -> Result<Self> {
        ...
        if !manifest_path.exists() {
            ...
            let (recm, records) = Manifest::recover(&manifest_path)?;
            ...
            for record in records {
                match record {
                    ...
                    ManifestRecord::NewMemtable(new_sst) => {
                        memtables.insert(new_sst);
                        next_sst_id = next_sst_id.max(new_sst);
                    }
                }
            }
                ...
                .chain(state.levels.iter().flat_map(|(_, files)| files))
            {
                let sst = SsTable::open(
                    *table,
                    ...
                    FileObject::open(&Self::path_of_sst_static(path, *table))?,
                )?;
                last_commit_ts = last_commit_ts.max(sst.max_ts());
                state.sstables.insert(*table, Arc::new(sst));
            }
            ...
            if options.enable_wal {
                for id in memtables.iter() {
                    ...
                    let max_ts = memtable.map.iter().map(|x| x.key().ts()).max().unwrap();
            ...
            recm.add_record_when_init(ManifestRecord::NewMemtable(state.memtable.id()))?;
            next_sst_id += 1;
            manifest = recm;
        }
    ...
    pub(crate) fn get(&self, _key: &[u8]) -> Result<Option<Bytes>> {
        let snapshot = {
        ...
        memtable_iters.push(Box::new(snapshot.memtable.scan(
            Bound::Included(KeySlice::from_slice(_key, key::TS_RANGE_BEGIN)),
            Bound::Included(KeySlice::from_slice(_key, key::TS_RANGE_END)),
        )));
    ...
    pub fn force_freeze_memtable(&self, _state_lock_observer: &MutexGuard<'_, ()>) -> Result<()> {
        ...
        let new_mem = if self.options.enable_wal {
            MemTable::create_with_wal(new_mem_id, self.path_of_wal(new_mem_id))?
        } else {
            MemTable::create(new_mem_id)
        };
        ...
        state
            .imm_memtables
            .insert(0, std::mem::replace(&mut state.memtable, Arc::new(new_mem)));

        *guard = Arc::new(state);

        self.manifest.as_ref().unwrap().add_record(
            _state_lock_observer,
            ManifestRecord::NewMemtable(new_mem_id),
        )?;
        self.sync_dir()?;
        Ok(())
    }
    ...
    pub fn force_flush_next_imm_memtable(&self) -> Result<()> {
        ...
        {
            let guard = self.state.read();
            mem = guard.imm_memtables.last().expect("No Imm Table").clone();
        };
        let mut sst_builder = SsTableBuilder::new(self.options.block_size);
        mem.flush(&mut sst_builder)?;
        let sst = Arc::new(sst_builder.build(
            mem.id(),
            Some(self.block_cache.clone()),
            self.path_of_sst(mem.id()),
        )?);
    ...
    pub(crate) fn scan(
        ...
        let (_lower, _upper) = Self::map_key_bound_plus_ts(lower, upper, TS_RANGE_BEGIN);
